...
typedef enum table_parse_op {
    ...
    GET_TABLE_MAX = GET_ASSEMBLY_REF,
} table_parse_op_t;
        ...
        while (*cur_op != DONE) {
            switch (*cur_op) {
                ...
                case GET_BLOB: {
                    ...
                    CHECK(idx < ctx->assembly->blob_size);
                    *(const uint8_t**)table = &ctx->assembly->blob[idx];
                    table += sizeof(uint8_t*);
                } break;

                case GET_GUID: {
                    uint32_t idx = ctx->long_guid_index ? FETCH_UINT32() : FETCH_UINT16();
                    CHECK(idx == 0 || idx - 1 < ctx->assembly->guids_count);
                    *(guid_t**)table = idx == 0 ? NULL : &ctx->assembly->guids[idx - 1];
                    table += sizeof(guid_t*);
                } break;

                case GET_STRING: {
                    uint32_t idx = ctx->long_guid_index ? FETCH_UINT32() : FETCH_UINT16();
                    CHECK(idx < ctx->assembly->strings_size);
                    *(const char**)table = &ctx->assembly->strings[idx];
...
static err_t resolve_coded_index_sizes(metadata_parse_ctx_t* ctx) {
    err_t err = NO_ERROR;
    ...
    for (int i = 0; i < ARRAY_LEN(ctx->long_coded_index); i++) {
        ...
        ctx->long_coded_index[i] = max_table_len > UINT16_MAX ? true : false;
    }
...
err_t metadata_parse(assembly_t* assembly, void* stream, size_t size, metadata_t* metadata) {
    ...
    for (int i = 0; i < streams; i++) {
        ...
        if (strcmp("#~", stream_header->name) == 0) {
            ...
            if (strcmp("#Strings", stream_header->name) == 0) {
                assembly->strings = target;
                assembly->strings_size = stream_header->size;
            } else if (strcmp("#US", stream_header->name) == 0) {
                assembly->us = target;
                assembly->us_size = stream_header->size;
            } else if (strcmp("#GUID", stream_header->name) == 0) {
                assembly->guids = target;
                assembly->guids_count = stream_header->size / 16;
            } else if (strcmp("#Blob", stream_header->name) == 0) {
                assembly->blob = target;
                assembly->blob_size = stream_header->size;
            } else {
    ...
    CHECK_AND_RETHROW(resolve_coded_index_sizes(&ctx));
...
void free_metadata(metadata_t* metadata) {
    for (int i = 0; i < ARRAY_LEN(metadata->tables); i++) {
        SAFE_FREE(metadata->tables[i].table);
    }
}
