...
// The region map
mem_region_t mi_regions[MI_REGION_MAX];

// Allocated regions
_Atomic(size_t) mi_regions_count; // = 0;        
...
mi_decl_nodiscard bool mi_is_in_heap_region(const void* p) mi_attr_noexcept {
  ...
  size_t count = mi_atomic_load_relaxed(&mi_regions_count);
  for (size_t i = 0; i < count; i++) {
    uint8_t* start = (uint8_t*)mi_atomic_load_ptr_relaxed(uint8_t, &mi_regions[i].start);
...
static size_t mi_memid_create(mem_region_t* region, mi_bitmap_index_t bit_idx) {
  ...
  size_t idx = region - mi_regions;
  mi_assert_internal(&mi_regions[idx] == region);
...
static bool mi_region_try_alloc_os(size_t blocks, bool commit, bool allow_large, mem_region_t** region, mi_bitmap_index_t* bit_idx, mi_os_tld_t* tld)
{
  ...
  if (mi_atomic_load_relaxed(&mi_regions_count) >= MI_REGION_MAX - 1) return false;
  ...
  // claim a fresh slot
  const size_t idx = mi_atomic_increment_acq_rel(&mi_regions_count);
  if (idx >= MI_REGION_MAX) {
    mi_atomic_decrement_acq_rel(&mi_regions_count);
...
static bool mi_region_try_claim(int numa_node, size_t blocks, bool allow_large, mem_region_t** region, mi_bitmap_index_t* bit_idx, mi_os_tld_t* tld)
{
  ...
  const size_t count = mi_atomic_load_relaxed(&mi_regions_count); // monotonic, so ok to be relaxed
...
void _mi_mem_collect(mi_os_tld_t* tld) {
  ...
  size_t rcount = mi_atomic_load_relaxed(&mi_regions_count);
  for (size_t i = 0; i < rcount; i++) {
    mem_region_t* region = &mi_regions[i];
    if (mi_atomic_load_relaxed(&region->info) != 0) {
      ...
      if (m == 0) {
        ...
        uint8_t* start = (uint8_t*)mi_atomic_load_ptr_acquire(uint8_t,&mi_regions[i].start);
        size_t arena_memid = mi_atomic_load_relaxed(&mi_regions[i].arena_memid);
        size_t commit = mi_atomic_load_relaxed(&mi_regions[i].commit);
        memset((void*)&mi_regions[i], 0, sizeof(mem_region_t));  // cast to void* to avoid atomic warning
